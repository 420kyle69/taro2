 ...
 */
var TaroPathComponent = TaroEventingClass.extend({
	classId: 'TaroPathComponent',
	componentId: 'path',
	 ...
	 * @param {TaroTileMap2d} val The tileMap to use for path calculations.
	 * @returns {*}
	 */
	 ...
	 * @param {TaroPathFinder} val The pathfinder class instance to use to generate paths.
	 * @returns {*}
	 */
	finder: function (val) {
		if (val !== undefined) {
	...
	set: function (fromX, fromY, fromZ, toX, toY, toZ, findNearest) {
		...
		var path = this._finder.generate(
			this._tileMap,
			new TaroPoint3d(fromX, fromY, fromZ),
			new TaroPoint3d(toX, toY, toZ),
			this._tileChecker,
			this._allowSquare,
			this._allowDiagonal,
			findNearest
	...
	/**
	 * Adds a path array containing path points (TaroPoint3d instances) to the points queue.
	 ...
	 * @return {TaroPoint3d} A new point representing the travelled from node.
	 */
	 ...
	 * @return {TaroPoint3d} A new point representing the travelling to node.
	 */
	getToPoint: function () {
	 ...
	 *         .addComponent(TaroPathComponent);
	 *
	...
	start: function (startTime) {
		if (!this._active) {
			...
			this._startTime = startTime || taro._currentTime;
	...
	pause: function () {
		this._active = false;
		this._paused = true;
		this._pauseTime = taro._currentTime;
	...
	_updateBehaviour: function (ctx) {
		var path = this.path;
		var currentTime = taro._currentTime;
	...
	_processDynamic: function (pointFrom, pointTo, destinationPoint) {
		...
		if (!self._tileChecker(tileCheckData, pointTo.x, pointTo.y, null, null, null, true)) {
			...
			newPathPoints = self._finder.generate(
				self._tileMap,
				new TaroPoint3d(pointFrom.x, pointFrom.y, pointFrom.z),
				new TaroPoint3d(destinationPoint.x, destinationPoint.y, destinationPoint.z),
				self._tileChecker,
				self._allowSquare,
				self._allowDiagonal,
				false
			...
			if (newPathPoints.length) {
				...
				self.emit('dynamicFail', [this, new TaroPoint3d(pointFrom.x, pointFrom.y, pointFrom.z), new TaroPoint3d(destinationPoint.x, destinationPoint.y, destinationPoint.z)]);
				self.clear();
			...
			if (currentPath.length) {
				if (currentPath && self._drawPath) {
					...
					for (pathPointIndex = 0; pathPointIndex < currentPath.length; pathPointIndex++) {
						...
						tracePathPoint = new TaroPoint3d(
	 ...
	 * @param {TaroPoint3d} p1 Vector start point
	 * @param {TaroPoint3d} p2 Vector end point
...
if (typeof (module) !== 'undefined' && typeof (module.exports) !== 'undefined') { module.exports = TaroPathComponent; }
