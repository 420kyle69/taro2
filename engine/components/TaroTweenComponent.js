/**
 * This component is already included in the TaroEngine (taro)
 * instance and is not designed for use in any other way!
 * It handles global tween processing on all tweening values.
 */
var TaroTweenComponent = TaroClass.extend({
	classId: 'TaroTweenComponent',
	componentId: 'tween',
	 ...
	 * @param {TaroTween} tween The tween to start.
	 * @return {Number} The index of the added tween or -1 on error.
	 */
	start: function (tween) {
		if (tween._startTime > taro._currentTime) {
			// The tween is scheduled for later
			// Push the tween into the TaroTweenComponent's _tweens array
		...
		// Enable tweening on the TaroTweenComponent
		this.enable();
		...
		if (targetObj) {
			...
			if (tween._currentStep === 0 && !newTime) {
				...
				if (tween._startTime === undefined) {
					tween._startTime = taro._currentTime;
				}
			} else {
	 ...
	 * @param {TaroTween} tween The tween to stop.
	 */
	stop: function (tween) {
	...
	update: function (ctx) {
		...
		if (thisTween._tweens && thisTween._tweens.length) {
			var currentTime = taro._tickStart;
...
if (typeof (module) !== 'undefined' && typeof (module.exports) !== 'undefined') { module.exports = TaroTweenComponent; }
