var TaroAudio = TaroEventingClass.extend({
	classId: 'TaroAudio',
	...
	_loaded: function (callback) {
		...
		taro.audio.decode(self._data, function (err, buffer) {
			if (!err) {
				self._buffer = buffer;
				taro.audio.log(`Audio file (${self._url}) loaded successfully`);
	...
	play: function () {
		...
		if (self._buffer) {
			bufferSource = taro.audio._ctx.createBufferSource();
			bufferSource.buffer = self._buffer;
			bufferSource.connect(taro.audio._ctx.destination);
