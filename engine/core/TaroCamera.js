 ...
 */
var TaroCamera = TaroEntity.extend({
	classId: 'TaroCamera',
	 ...
	 * will be limited to using an TaroRect instance.
	 * @param {TaroRect=} rect
	 * @return {*}
	 */
	limit: function (rect) {
	 ...
	 * the specified time.
	 * @param {TaroPoint3d} point The point describing the co-ordinates to pan to.
	 ...
	 * the specified time.
	 * @param {TaroPoint3d} point The point describing the co-ordinates to pan by.
	 * @param {Number} durationMs The number of milliseconds to span the pan operation over.
	 * @param {String=} easing Optional easing method name.
	 */
	 ...
	 * @param {TaroEntity} entity The entity to look at.
	 ...
	 */
	tick: function (ctx) {
		if (taro.isClient && !taro.client.cameraEnabled) return;

		// Process any behaviours assigned to the camera
		this._processTickBehaviours(); // commented out because of redundancy. it's already declared in TaroEntity.js (jaeyun apr 2017)
			...
			if (this.hasOwnProperty(i) && this[i] !== undefined) {
				switch (i) {
					case '_trackTranslateTarget':
						str += `.trackTranslate(taro.$('${this._trackTranslateTarget.id()}'), ${this.trackTranslateSmoothing()})`;
						break;
					case '_trackRotateTarget':
						str += `.trackRotate(taro.$('${this._trackRotateTarget.id()}'), ${this.trackRotateSmoothing()})`;
						break;
				}
			}
		}
