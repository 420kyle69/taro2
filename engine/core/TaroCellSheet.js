 ...
 * width and height.
 */
var TaroCellSheet = TaroTexture.extend({
	classId: 'TaroCellSheet',
	TaroSpriteSheet: true,
	...
	init: function (url, horizontalCells, verticalCells, shouldScale, spriteCellSheet, entity) {
		...
		var callbackIfNotLoaded = function (err) {
			if (err && entity) {
				...
				var defaultUrl = taro.game.getAsset(`${entity._category}Types`, entityType).cellSheet.url;
				TaroTexture.prototype.init.call(self, defaultUrl);
			}
		};
		TaroTexture.prototype.init.call(this, url, spriteCellSheet && entity ? callbackIfNotLoaded : undefined);
	},
	...
	_textureLoaded: function () {
		...
		TaroTexture.prototype._textureLoaded.call(this);
	},
	...
	_applyCells: function () {
		...
		if (this.image) {
			...
			if (this._cellRows && this._cellColumns) {
				...
				if (this.shouldScale) {
					imgWidth = this._sizeX * taro.scaleMapDetails.scaleFactor.x;
					imgHeight = this._sizeY * taro.scaleMapDetails.scaleFactor.y;
				}
				rows = this._cellRows;
