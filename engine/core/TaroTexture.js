 ...
 */
var TaroTexture = TaroEventingClass.extend({
	classId: 'TaroTexture',
	TaroTexture: true,

	/**
	 * Constructor for a new TaroTexture.
	...
	init: function (urlOrObject, callback) {
		...
		if (taro.isServer) {
			this.log('Cannot create a texture on the server. Textures are only client-side objects. Please alter your code so that you don\'t try to load a texture on the server-side using something like an if statement around your texture laoding such as "if (taro.isClient) {}".', 'error');
			return this;
		}
		/* CEXCLUDE */
		...
		this._cells = [];
		this._smoothing = taro._globalSmoothing;
	...
	id: function (id) {
		...
		if (!this._id) {
			...
			if (this._url) {
				...
				this._id = taro.newIdFromString(this._url);
			} else {
	...
	_loadImage: function (imageUrl) {
		...
		if (taro.isClient) {
			...
			taro.textureLoadStart(imageUrl, this);

			// Check if the image url already exists in the image cache
			if (!taro._textureImageStore[imageUrl]) {
				// Image not in cache, create the image object
				image = taro._textureImageStore[imageUrl] = this.image = this._originalImage = new Image();
				image._taroTextures = image._taroTextures || [];

				// Add this texture to the textures that are using this image
				image._taroTextures.push(this);
				...
				image.onload = function () {
					...
					// Log success
					taro.devLog(`Texture image (${imageUrl}) loaded successfully`);
				...
				image = this.image = this._originalImage = taro._textureImageStore[imageUrl];
	...
	_textureLoaded: function () {
		...
		setTimeout(function () {
			...
			taro.textureLoadEnd(self.image.src, self);
		}, 5);
	},
	...
	_loadScript: function (scriptUrl) {
		var textures = taro.textures;
		var rs_sandboxContext;
		var self = this;
		var scriptElem;

		taro.textureLoadStart(scriptUrl, this);
		...
		if (taro.isClient) {
			...
			scriptElem.onload = function (data) {
				...
				self._loaded = true;
				self.emit('loaded');
				taro.textureLoadEnd(scriptUrl, self);
			};
	 ...
	 * @param {TaroTexture} tex The new texture to paint to.
	...
	_textureFromCell: function (tex, indexOrId) {
		...
		if (this._cells[index]) {
			// Create a new TaroTexture, then draw the existing cell
	 ...
	 * @param {TaroEntity} entity The entity that this texture is
	 * being drawn for.
	 */
	render: function (ctx, entity) {
		...
		if (entity._cell !== null) {
			...
			// TODO: track of the value and evaluate first before changing?
			if (!this._smoothing) {
				taro._ctx.imageSmoothingEnabled = false;
				// taro._ctx.webkitImageSmoothingEnabled = false;
				taro._ctx.mozImageSmoothingEnabled = false;
			} else {
				taro._ctx.imageSmoothingEnabled = true;
				// taro._ctx.webkitImageSmoothingEnabled = true;
				taro._ctx.mozImageSmoothingEnabled = true;
			}
			...
			if (this._mode === 0) {
				...
				var poly = entity._renderPos; // Render pos is calculated in the TaroEntity.aabb() method
	 ...
	 * @return {TaroTexture} A new, distinct texture with the same attributes
	...
	destroy: function () {
		...
		if (this.image && this.image._taroTextures) {
			this.image._taroTextures.pull(this);
		}

		// Remove the texture from the texture store
		taro._textureStore.pull(this);
...
if (typeof (module) !== 'undefined' && typeof (module.exports) !== 'undefined') { module.exports = TaroTexture; }
