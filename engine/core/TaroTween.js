 ...
 */
var TaroTween = TaroClass.extend({
	classId: 'TaroTween',
	...
	easing: function (methodName) {
		if (methodName !== undefined) {
			if (taro.tween.easing[methodName]) {
				this._easing = methodName;
			} else {
				this.log('The easing method you have selected does not exist, please use a valid easing method. For a list of easing methods please inspect taro.tween.easing from your console.', 'error', taro.tween.easing);
			}
		}
	...
	start: function (timeMs) {
		if (timeMs !== undefined) {
			this.startTime(timeMs + taro._currentTime);
		}

		taro.tween.start(this);
...
if (typeof (module) !== 'undefined' && typeof (module.exports) !== 'undefined') { module.exports = TaroTween; }
