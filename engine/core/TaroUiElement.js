 ...
 * than standard TaroEntity instances but provide a rich set of extra
 * positioning and styling methods as well as reacting to styles
 * defined using the TaroUiManagerComponent.
 */
var TaroUiElement = TaroUiEntity.extend({
	classId: 'TaroUiElement',
	...
	styleClass: function (name) {
		if (name !== undefined) {
			...
			if (this._styleClass && this._styleClass !== name) {
				...
				taro.ui.unRegisterElementStyle(this);
			}
			...
			taro.ui.registerElementStyle(this);
	...
	_processStyle: function (styleName, state) {
		if (styleName) {
			...
			// Basic
			var styleData = taro.ui.style(styleName);
			if (styleData) {
	 ...
	 * ui style methods available in the ./engine/extensions/TaroUi* files.
	 *
	 ...
	 */
	destroy: function () {
		taro.ui.unRegisterElement(this);
		TaroUiEntity.prototype.destroy.call(this);
	}
});

if (typeof (module) !== 'undefined' && typeof (module.exports) !== 'undefined') { module.exports = TaroUiElement; }
