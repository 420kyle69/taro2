...
var TaroViewport = TaroEntity.extend(
	...
	],
	{
		...
		viewArea: function () {
			...
			return new TaroRect(camTrans.x - width / 2, camTrans.y - height / 2, width, height);
		},
		...
		tick: function (ctx, scene) {
			...
			if (this._scene) {
				...
				ctx.translate(
					((this._bounds2d.x / 2) | 0) + taro._translate.x,
					((this._bounds2d.y / 2) | 0) + taro._translate.y
				);
		 ...
		 * screen location of an taro entity. The returned values indicate the center
			...
			if (arr) {
				...
				while (arrCount--) {
					...
					if (obj._shouldRender !== false) {
						if (
							(obj._classId !== 'TaroScene2d' && !this._drawBoundsLimitId && !this._drawBoundsLimitCategory) ||
							...
							if (typeof obj.aabb === 'function') {
								...
								if (aabb) {
									...
									if (this._drawBoundsData && (obj._drawBounds || obj._drawBoundsData === undefined)) {
										...
										ctx.fillText(
											`X: ${obj._translate.x.toFixed(2)}, ` +
												`Y: ${obj._translate.y.toFixed(2)}, ` +
												`Z: ${obj._translate.z.toFixed(2)}`,
											aabb.x + aabb.width + 3,
											aabb.y + 20
										);
