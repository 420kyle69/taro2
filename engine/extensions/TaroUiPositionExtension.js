var TaroUiPositionExtension = {
			...
			if (px === null) {
				...
				if (typeof px === 'string') {
					if (this._parent) {
						...
						// We don't have a parent so use the main canvas
						// as a reference
						var parentWidth = taro._bounds2d.x;
						var val = parseInt(px, 10);
			...
			if (px === null) {
				...
				if (typeof px === 'string') {
					if (this._parent) {
						...
						// We don't have a parent so use the main canvas
						// as a reference
						var parentHeight = taro._bounds2d.y;
						var val = parseInt(px, 10);
	...
	_updateUiPosition: function () {
		if (this._parent) {
			...
			/* if (this._ignoreCamera && taro._currentCamera) {
				// Handle cam ignore when calculating
				parentGeom = parentGeom.dividePoint(taro._currentCamera._scale);
			} */
