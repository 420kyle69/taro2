	...
	extendParams(materialParams, materialDef, parser) {
		...
		if (metallicRoughness) {
			...
			if (metallicRoughness.baseColorTexture !== undefined) {
				pending.push(
					parser.assignTexture(materialParams, 'map', metallicRoughness.baseColorTexture, THREE.SRGBColorSpace)
				);
			}
		}
...
class GLTFMaterialsSheenExtension {
	...
	extendMaterialParams(materialIndex, materialParams) {
		...
		if (extension.sheenColorTexture !== undefined) {
			pending.push(
				parser.assignTexture(materialParams, 'sheenColorMap', extension.sheenColorTexture, THREE.SRGBColorSpace)
			);
		}
...
class GLTFMaterialsSpecularExtension {
	...
	extendMaterialParams(materialIndex, materialParams) {
		...
		materialParams.specularColor = new THREE.Color().setRGB(
			colorArray[0],
			colorArray[1],
			colorArray[2],
			THREE.LinearSRGBColorSpace
...
class GLTFParser {
	...
	loadMaterial(materialIndex) {
		...
		if (materialDef.emissiveTexture !== undefined && materialType !== THREE.MeshBasicMaterial) {
			pending.push(
				parser.assignTexture(materialParams, 'emissiveMap', materialDef.emissiveTexture, THREE.SRGBColorSpace)
			);
		}
