class PhaserRegion extends PhaserEntity {
    ...
    constructor(scene, entity) {
        ...
        const devModeScene = this.devModeScene = taro.renderer.scene.getScene('DevMode');
    ...
    getLabel() {
        if (!this.label) {
            ...
            const label = this.label = scene.add.bitmapText(0, 0, BitmapFontManager.font(scene, 'Verdana', false, taro.game.data.settings
    ...
    transform() {
        ...
        if (this.devModeOnly) {
            ...
            graphics.fillStyle(Number(`0x${stats.inside.substring(1)}`), 
            // between 0 and 1 or we default
