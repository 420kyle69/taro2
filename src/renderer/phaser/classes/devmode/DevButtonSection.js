class DevButtonSection {
    constructor(devModeTools, name, y, height) {
        this.name = name;
        this.buttons = [];
        this.y = y;
        this.height = height;
        ...
        if (name) {
            ...
            const label = this.label = scene.add.bitmapText(w / 2, y - h / 3, BitmapFontManager.font(scene, 'Verdana', true, false, '#000000'), this.name, 16);
            label.scale = 0.8;
            label.setOrigin(0.5);
    ...
    addButton(button, array = this.buttons) {
