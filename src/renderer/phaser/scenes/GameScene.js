class GameScene extends PhaserScene {
    ...
    init() {
        ...
        taro.client.on('change-filter', (data) => {
            this.changeTextureFilter(data.filter);
        });
        taro.client.on('updateMap', () => {
            this.updateMap();
        });
        taro.client.on('create-unit', (unit) => {
            new PhaserUnit(this, unit);
        });
        taro.client.on('create-item', (item) => {
            new PhaserItem(this, item);
        });
        taro.client.on('create-projectile', (projectile) => {
            new PhaserProjectile(this, projectile);
        });
        taro.client.on('create-region', (region) => {
            new PhaserRegion(this, region);
        });
        taro.client.on('create-ray', (data) => {
            new PhaserRay(this, data.start, data.end, data.config);
        });
        taro.client.on('create-particle', (particle) => {
        ...
        taro.client.on('position-camera', (x, y) => {
            x -= camera.width / 2;
            y -= camera.height / 2;
            camera.setScroll(x, y);
        });
        taro.client.on('deadzone-camera', (width, heigth) => {
            camera.setDeadzone(width, heigth);
        });
        taro.client.on('instant-move-camera', (x, y) => {
        ...
        taro.client.on('unit-position', (x, y) => {
            var _a;
            (_a = this.visibility) === null || _a === void 0 ? void 0 : _a.moveCenter(x, y);
        });
    }
    preload() {
        ...
        data.map.tilesets.forEach((tileset) => {
            ...
            this.load.once(`filecomplete-image-${key}`, () => {
                ...
                const canvas = this.extrude(tileset, texture.getSourceImage());
                if (canvas) {
    ...
    extrude(tileset, sourceImage, extrusion = 2, color = '#ffffff00') {
    ...
    setResolution(resolution, setResolutionCoef) {
